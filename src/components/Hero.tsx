...
export const Hero = () => {
  ...
  const codeLines = [
    ...
    { t: "    self.name = ", k: `"${t.hero.name}"` },
    { t: "    self.role = ", k: `"${locale === "ar" ? "مهندس ذكاء اصطناعي" : "Full Stack Developer"}"` },
    { t: "    self.skills = ", k: '{ "AI/ML", "Frontend", "Backend" }' },
    { t: "✓ ", k: t.hero.initialized },
  ];
    ...
    <section ref={ref} id="top" className="relative min-h-[100svh] flex items-center justify-center overflow-hidden pt-28 pb-16">
      ...
      <motion.div style={{ opacity }} className="container relative grid lg:grid-cols-[1.15fr_1fr] gap-12 items-center">
        <div>
          ...
          >
            <span className="font-medium text-foreground"><HeroTyping text={t.hero.name} /> {t.hero.headingLead}</span>
            <br />
            <span className="font-semibold text-foreground">{t.hero.headingPrimary} </span>
            <span className="font-semibold text-gradient">{t.hero.headingAccent}</span>
          </motion.h1>
          ...
          >
            <MagneticButton as="a" href={localizedPath("#contact")} className="group w-full sm:w-auto">
              {t.hero.hireMe} <ArrowRight size={18} className={`${locale === "ar" ? "mr-2 rotate-180" : "ml-2"} transition-transform duration-300 group-hover:translate-x-1`} />
            </MagneticButton>
            <MagneticButton as="a" href={localizedPath("#projects")} variant="ghost" className="w-full sm:w-auto">
              {t.hero.viewProjects}
            </MagneticButton>
          </motion.div>
          ...
          >
            ...
            <div className="hidden sm:block opacity-60">{t.hero.stack}</div>
          </motion.div>
        </div>
          ...
          <div className="relative z-10 glass-strong rounded-2xl overflow-hidden shadow-2xl shadow-black/50">
            <div className="flex items-center gap-2 px-4 py-3 border-b border-foreground/5 bg-black/20">
              ...
              <span className="ml-2 flex items-center gap-1.5 text-xs font-mono text-muted-foreground">
                <Terminal size={12} /> {t.hero.terminalFile}
              </span>
            </div>
